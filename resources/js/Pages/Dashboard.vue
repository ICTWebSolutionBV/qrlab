...
<template>
    ...
    <AppLayout>
        ...
        <!-- Grid / Card view -->
        <div v-else-if="viewMode === 'grid'" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-3">
            <div v-for="qr in qrCodes.data" :key="qr.id"
                ...
                <div class="flex items-start justify-between mb-2 gap-2">
                    <div class="min-w-0">
                        <h3 class="font-semibold text-sm text-gray-900 dark:text-white truncate">{{ qr.name }}</h3>
                        <p class="text-xs text-gray-500 dark:text-gray-400 truncate">{{ subtitle(qr) }}</p>
                    </div>
                    <div class="flex flex-col items-end gap-0.5 shrink-0">
                        <span class="text-[10px] px-1.5 py-0.5 rounded-full bg-primary-50 dark:bg-primary-900/20 text-primary-700 dark:text-primary-400 font-medium">
                            {{ typeLabel(qr) }}
                        </span>
                        <span v-if="qr.tracking_enabled" class="text-[10px] text-gray-400 dark:text-gray-500">
                ...
                <div class="flex items-center gap-1 mt-2">
                    <button @click="showQr(qr)" title="Show QR"
                        class="flex-1 inline-flex items-center justify-center gap-1 px-2 py-1.5 text-[11px] font-medium text-gray-700 dark:text-gray-200 bg-gray-100 dark:bg-gray-800 rounded-md hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"/></svg>
                        Show
                    </button>
                    <Link :href="route('qr.edit', qr.id)" title="Edit"
                        class="p-1.5 text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-800 rounded-md hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">
                    ...
                    <Link v-if="qr.tracking_enabled" :href="route('qr.analytics', qr.id)" title="View scan analytics"
                        class="p-1.5 text-primary-700 dark:text-primary-400 bg-primary-50 dark:bg-primary-900/20 rounded-md hover:bg-primary-100 dark:hover:bg-primary-900/40 transition-colors">
                    ...
                    <button @click="confirmDelete(qr)" title="Delete QR"
                        class="p-1.5 text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 rounded-md hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors">
